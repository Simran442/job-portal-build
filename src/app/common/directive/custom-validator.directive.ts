...
export class CustomValidators {
    ...
    /**
     * Function to validate the comission
     * @param c 
     */
    static commisionBetween(c: FormControl): ValidationErrors {
        const value = c.value;
        if (value) {
            let isValid = true;
            const message = {
                vaildEmail: {
                    message: 'Comissão não autorizada. Entre em contato com nosso equipe de atendimento'
                }
            };
            if (value > 0 && value <= 15) {
                isValid = true;
